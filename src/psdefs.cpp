...
"/@dodraw false store :show/@dodraw true store}{pop}ifelse}put @SD/awidthshow{{"
...
"/widthshow{0 0 3 -1 roll pstack awidthshow}put @SD/ashow{0 0 0 6 3 roll awidth"
"show}put @SD/newpath{:newpath 0 1(newpath)prcmd}put @SD/stroke{@dodraw{prcolor"
" 1 1(newpath)prcmd prpath 0(stroke)prcmd :newpath}{:stroke}ifelse}put @SD/fill"
"{@dodraw{prcolor 1 1(newpath)prcmd prpath 0(fill)prcmd :newpath}{:fill}ifelse}"
"put @SD/eofill{@dodraw{prcolor 1 1(newpath)prcmd prpath 0(eofill)prcmd :newpat"
"h}{:eofill}ifelse}put @SD/clip{:clip 0 1(newpath)prcmd prpath 0(clip)prcmd}put"
" @SD/eoclip{:eoclip 1 1(newpath)prcmd prpath 0(eoclip)prcmd}put @SD/shfill{beg"
...
"cmd pop}def/@setpagedevice{pop<<>>/setpagedevice sysexec[1 0 0 -1 0 0]setmatri"
"x newpath 0(setpagedevice)prcmd}def/prcolor{currentrgbcolor 3(setrgbcolor)prcm"
"d}def/printgstate{matrix currentmatrix aload pop 6(setmatrix)prcmd applyscalev"
"als currentlinewidth 1(setlinewidth)prcmd currentlinecap 1(setlinecap)prcmd cu"
"rrentlinejoin 1(setlinejoin)prcmd currentmiterlimit 1(setmiterlimit)prcmd curr"
"entrgbcolor 3(setrgbcolor)prcmd currentdash mark 3 1 roll exch aload length 1 "
"add -1 roll counttomark(setdash)prcmd pop}def/setgstate{/setgstate sysexec pri"
"ntgstate}def/save{@UD begin/@saveID vmstatus pop pop def end :save @saveID 1(s"
"ave)prcmd}def/restore{:restore printgstate @UD/@saveID known{@UD begin @saveID"
" end}{0}ifelse 1(restore)prcmd}def/gsave 0 defpr/grestore{:grestore printgstat"
"e 0(grestore)prcmd}def/grestoreall{:grestoreall setstate 0(grestoreall)prcmd}d"
"ef/rotate{dup type/arraytype ne{dup 1(rotate)prcmd}if/rotate sysexec applyscal"
"evals}def/scale{dup type/arraytype ne{2 copy 2(scale)prcmd}if/scale sysexec ap"
"plyscalevals}def/translate{dup type/arraytype ne{2 copy 2(translate)prcmd}if/t"
"ranslate sysexec}def/setmatrix{dup/setmatrix sysexec aload pop 6(setmatrix)prc"
"md applyscalevals}def/initmatrix{matrix setmatrix}def/concat{matrix currentmat"
"rix matrix concatmatrix setmatrix}def/makepattern{gsave<</mx 3 -1 roll>>begin "
"dup/XUID[1000000 @patcnt]put mx/makepattern sysexec dup dup begin PatternType "
"@patcnt BBox aload pop XStep YStep PaintType mx aload pop 15(makepattern)prcmd"
" :newpath matrix setmatrix PaintProc 0 1(makepattern)prcmd end/@patcnt @patcnt"
" 1 add store end grestore}def/setpattern{begin PatternType 1 eq{PaintType 1 eq"
"{XUID aload pop exch pop 1}{:gsave[currentcolorspace aload length -1 roll pop]"
"setcolorspace/setcolor sysexec XUID aload pop exch pop currentrgbcolor :gresto"
"re 4}ifelse(setpattern)prcmd}{/setpattern sysexec}ifelse end}def/setcolor{dup "
"type/dicttype eq{setpattern}{/setcolor sysexec/currentrgbcolor sysexec setrgbc"
"olor}ifelse}def/setgray 1 defpr/setcmykcolor 4 defpr/sethsbcolor 3 defpr/setrg"
"bcolor 3 defpr/.setopacityalpha{dup/.setopacityalpha sysexec 1(setopacityalpha"
")prcmd}def/.setshapealpha{dup/.setshapealpha sysexec 1(setshapealpha)prcmd}def"
"/.setblendmode{dup/.setblendmode sysexec<</Normal 0/Compatible 0/Multiply 1/Sc"
"reen 2/Overlay 3/SoftLight 4/HardLight 5/ColorDodge 6/ColorBurn 7/Darken 8/Lig"
"hten 9/Difference 10/Exclusion 11/Hue 12/Saturation 13/Color 14/Luminosity 15/"
"CompatibleOverprint 16>>exch get 1(setblendmode)prcmd}def/@pdfpagecount{GS_PDF"
"_ProcSet begin pdfdict begin(r)file pdfopen begin pdfpagecount currentdict pdf"
"close end end end}def/@pdfpagebox{GS_PDF_ProcSet begin pdfdict begin(r)file pd"
"fopen begin dup dup 1 lt exch pdfpagecount gt or{pop}{pdfgetpage/MediaBox pget"
" pop aload pop}ifelse currentdict pdfclose end end end}def DELAYBIND{.bindnow}"
"if ";
