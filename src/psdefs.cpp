...
"cmd}def/printgstate{matrix currentmatrix aload pop 6(setmatrix)prcmd applyscal"
"evals currentlinewidth 1(setlinewidth)prcmd currentlinecap 1(setlinecap)prcmd "
"currentlinejoin 1(setlinejoin)prcmd currentmiterlimit 1(setmiterlimit)prcmd cu"
"rrentrgbcolor 3(setrgbcolor)prcmd currentdash mark 3 1 roll exch aload length "
"1 add -1 roll counttomark(setdash)prcmd pop}def/setgstate{/setgstate sysexec p"
"rintgstate}def/save{@UD begin/@saveID vmstatus pop pop def end :save @saveID 1"
"(save)prcmd}def/restore{:restore printgstate @UD/@saveID known{@UD begin @save"
"ID end}{0}ifelse 1(restore)prcmd}def/gsave 0 defpr/grestore{:grestore printgst"
"ate 0(grestore)prcmd}def/grestoreall{:grestoreall setstate 0(grestoreall)prcmd"
"}def/rotate{dup type/arraytype ne{dup 1(rotate)prcmd}if/rotate sysexec applysc"
"alevals}def/scale{dup type/arraytype ne{2 copy 2(scale)prcmd}if/scale sysexec "
"applyscalevals}def/translate{dup type/arraytype ne{2 copy 2(translate)prcmd}if"
"/translate sysexec}def/setmatrix{dup/setmatrix sysexec aload pop 6(setmatrix)p"
"rcmd applyscalevals}def/initmatrix{matrix setmatrix}def/concat{matrix currentm"
...
"5/CompatibleOverprint 16>>exch get 1(setblendmode)prcmd}def/@pdfpagecount{GS_P"
"DF_ProcSet begin pdfdict begin(r)file pdfopen begin pdfpagecount currentdict p"
"dfclose end end end}def/@pdfpagebox{GS_PDF_ProcSet begin pdfdict begin(r)file "
"pdfopen begin dup dup 1 lt exch pdfpagecount gt or{pop}{pdfgetpage/MediaBox pg"
"et pop aload pop}ifelse currentdict pdfclose end end end}def DELAYBIND{.bindno"
"w}if ";
